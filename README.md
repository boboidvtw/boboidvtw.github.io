...
### 函數繪圖 ⭐ 新功能
- 📈 **Canvas 2D 函數圖表** — 自動渲染二維函數曲線
- 🎯 **X 軸變數選擇** — 多變數公式自動列表支援
- 📊 **動態Y軸** — 根據計算結果自動調整範圍
- 🔍 **互動式 Hover** — 滑鼠移過曲線即時顯示 (x, f(x)) 座標
- ⚙️ **靈活控制** — X 範圍、採樣點數、其他變數固定值可自訂
- 🌍 **完全離線** — 零外部依賴，純 JavaScript 實現
...
3. **配置繪圖參數**：
   - 選擇 **X 軸變數**（下拉選單）
   - 設定 **X 最小值** / **X 最大值**（預設 -10 到 10）
   - 調整 **採樣點數**（10–2000，預設 200）
   - 輸入 **其他變數固定值**（若公式有多個變數）
4. 圖表自動重繪（或按「繪製」手動重繪）
5. **互動式查看**：將滑鼠移過曲線，即時顯示 (x, f(x)) 座標
6. 按「關閉」回到計算面板

**範例**：
- 二次函數 `f(x) = a*x²+b*x+c`，設 a=1, b=1, c=1 → 拋物線
- 勾股定理 `f(a) = √(a²+b²)`，設 b=1 → V形曲線
...
**切換語言**：點擊介面右上角「繁體中文」語言選單，選擇欲使用的語言。所有 UI 元素（按鈕、標籤、提示、繪圖介面）會即時切換。
